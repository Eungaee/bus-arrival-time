...
posURL = 'http://ws.bus.go.kr/api/rest/buspos/getBusPosByRtid'  #노선ID로 차량들의 위치정보를 조회
...
def getBusPos(busNum):
    routeId = getRouteId(busNum)
    params = {'serviceKey': key, 'busRouteId': routeId, 'resultType': 'json'}
    response = requests.get(posURL, params).content.decode('utf-8')
    responseJson = json.loads(response)
    itemList = responseJson.get('msgBody').get('itemList')
    gpsXYList = []
    for i in range(len(itemList)):
        x = itemList[i].get('gpsY')
        y = itemList[i].get('gpsX')
        nextStId = itemList[i].get('nextStId')
        gpsXY = [x, y, nextStId]
        gpsXYList.append(gpsXY)
...
def main():
    ...
    stationValue = int(input(f"1: {nextStationName[0]}\n2: {nextStationName[1]}\n가고싶은 다음정류장을 선택하세요: "))
    if stationValue == 1:
